...
return true;}
function insertHtmlAtCaret(html){var targetSelector=(arguments.length>1)?arguments[1]:null;var sel,range;if(window.getSelection){sel=window.getSelection();if(targetSelector){console.log($(sel.anchorNode).closest(targetSelector).length);targetSelectorPrefix=targetSelector.slice(0,1);targetSelectorName=targetSelector.slice(1);if(targetSelectorPrefix=='.'){if(!$(sel.anchorNode).hasClass(targetSelectorName)||!$(sel.anchorNode).closest(targetSelector).length)return false;}else if(targetSelectorPrefix=='#'){if(!$(sel.anchorNode).attr('id')!=targetSelectorName||!$(sel.anchorNode).closest(targetSelector).length)return false;}}
