...
return true;}
function insertHtmlAtCaret(html){var targetSelector=(arguments.length>1)?arguments[1]:null;var sel,range;if(window.getSelection){sel=window.getSelection();console.log(sel);if(targetSelector){targetSelectorPrefix=targetSelector.slice(0,1);targetSelectorName=targetSelector.slice(1);console.log(targetSelectorPrefix);console.log(targetSelectorName);console.log(sel.anchorNode);console.log(11111);if(targetSelectorPrefix=='.'){if(!$(sel.anchorNode).hasClass(targetSelectorName))return false;console.log(2222);}else if(targetSelectorPrefix=='#'){if(!$(sel.anchorNode).attr('id')!=targetSelectorName)return false;}}
console.log(sel.getRangeAt);console.log(sel.rangeCount);if(sel.getRangeAt&&sel.rangeCount){console.log(333);range=sel.getRangeAt(0);console.log(range);range.deleteContents();var el=document.createElement("div");el.innerHTML=html;var frag=document.createDocumentFragment(),node,lastNode;while((node=el.firstChild)){lastNode=frag.appendChild(node);}
range.insertNode(frag);console.log(lastNode);if(lastNode){range=range.cloneRange();range.setStartAfter(lastNode);range.collapse(true);sel.removeAllRanges();sel.addRange(range);}}}else if(document.selection&&document.selection.type!="Control"){document.selection.createRange().pasteHTML(html);}else{return false;}
return true;}
