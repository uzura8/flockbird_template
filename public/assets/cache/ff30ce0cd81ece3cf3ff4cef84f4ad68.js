...
return true;}
function insertHtmlAtCaret(html){var targetSelector=(arguments.length>1)?arguments[1]:null;var sel,range;if(window.getSelection){sel=window.getSelection();if(targetSelector&&!checkCursorOnTarget(sel.anchorNode,targetSelector))return false;if(sel.getRangeAt&&sel.rangeCount){range=sel.getRangeAt(0);range.deleteContents();var el=document.createElement("div");el.innerHTML=html;var frag=document.createDocumentFragment(),node,lastNode;while((node=el.firstChild)){lastNode=frag.appendChild(node);}
...
return true;}
function checkCursorOnTarget(selectedNode,targetSelector){if($(sel.anchorNode).closest(targetSelector).length)return true;targetSelectorPrefix=targetSelector.slice(0,1);targetSelectorName=targetSelector.slice(1);if(targetSelectorPrefix=='.'){if($(sel.anchorNode).hasClass(targetSelectorName))return true;}else if(targetSelectorPrefix=='#'){if($(sel.anchorNode).attr('id')==targetSelectorName)return true;}
return false;}
